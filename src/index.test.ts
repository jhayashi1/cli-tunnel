...
const getTicket = async (baseUrl: string, token: string): Promise<string> => {
...
const spawnServer = async (
    ...
    const state = {resolved: false, stdout: ''};

    return await new Promise<ServerInfo>((resolve, reject) => {
...
describe('HTTP Server', () => {
    ...
    it('11 — POST /api/auth/ticket with valid token returns 200 + ticket', async () => {
        ...
        expect(res.status).toBe(200);
        const body = (await res.json()) as { expires: number; ticket: string; };
...
describe('Rate Limiting', () => {
    ...
    it('12 — returns 429 after 35+ rapid API requests', async () => {
        const requests = Array.from({length: 35}, async () =>
            await fetch(`${rlServer.baseUrl}/api/sessions`)
        );
...
describe('WebSocket', () => {
    ...
    afterEach(async () => {
        await Promise.all(openSockets.map(async (ws) => await closeWs(ws)));
